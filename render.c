...
/*   Updated: 2024/09/30 23:00:12 by licohen          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int render(t_fractal *fractal)
{
    ...
    // if (!ft_strncmp(fractal->name, "fern", 4))
    // {
    //     fern(0, 0, fractal);
    //     mlx_put_image_to_window(fractal->disp, fractal->win, fractal->img.img, 0, 0);
    //     return (0);
    // }
    y = -1;
    while (++y < HEIGHT)
...
{
    int index;
    double scaled_x = (x * (fractal->vars.dist_x / WIDTH)) + fractal->vars.x_min;
    double scaled_y = (y * (fractal->vars.dist_y / HEIGHT)) + fractal->vars.y_min;
